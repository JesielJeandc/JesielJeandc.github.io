...
function addOrder() {
    ...
    if (parseFloat(qty1.value) > 0) {
        var order1 = qty1.value + " pc/s x " + price1.textContent + " Php " + product1.textContent + " ------- Php " + (parseFloat(qty1.value) * parseFloat(price1.textContent)).toFixed(2);
        carts.textContent += order1 + "\n";
        orderTotal += parseFloat(qty1.value) * parseFloat(price1.textContent);
    }

    if (parseFloat(qty2.value) > 0) {
        var order2 = qty2.value + " pc/s x " + price2.textContent + " Php " + product2.textContent + " ------- Php " + (parseFloat(qty2.value) * parseFloat(price2.textContent)).toFixed(2);
    ...
    if (parseFloat(qty3.value) > 0) {
        ...
        orderTotal += parseFloat(qty3.value) * parseFloat(price2.textContent);
    }
    if (parseFloat(qty4.value) > 0) {
        var order4 = qty4.value + " pc/s x " + price4.textContent + " Php " + product4.textContent + " ------- Php " + (parseFloat(qty4.value) * parseFloat(price4.textContent)).toFixed(2);
        carts.textContent += order4 + "\n";
        orderTotal += parseFloat(qty4.value) * parseFloat(price4.textContent);
    }
    if (parseFloat(qty5.value) > 0) {
        var order5 = qty5.value + " pc/s x " + price5.textContent + " Php " + product5.textContent + " ------- Php " + (parseFloat(qty5.value) * parseFloat(price5.textContent)).toFixed(2);
        carts.textContent += order5 + "\n";
        orderTotal += parseFloat(qty5.value) * parseFloat(price5.textContent);
    }
    if (parseFloat(qty6.value) > 0) {
        var order6 = qty6.value + " pc/s x " + price6.textContent + " Php " + product6.textContent + " ------- Php " + (parseFloat(qty6.value) * parseFloat(price6.textContent)).toFixed(2);
   ...
   total.value = orderTotal.toFixed(2);
    ...
    if (parseFloat(cash.value) >= orderTotal) {
        change.value = (parseFloat(cash.value) - orderTotal).toFixed(2);
    } else {
        change.value = "0.00";
    }
}
